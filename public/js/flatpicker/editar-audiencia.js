...
const initialHoraInicio = $('#hora_audiencia').val().split(':').slice(0, 2).join(':'); // HH:mm
const initialHoraFin = $('#hora_fin_audiencia').val().split(':').slice(0, 2).join(':');   // HH:mm
...
// ---------------------- Hora inicio ----------------------
$('#hora_audiencia').val(initialHoraInicio || formatTime(today));
$('#hora_fin_audiencia').val(initialHoraFin || formatTime(new Date(today.getTime() + 60 * 60 * 1000)));
...
$('#hora_audiencia').clockpicker({
    ...
    let hourFin = parseInt($('#hora_fin_audiencia').val().split(':')[0]);
    let minuteFin = parseInt($('#hora_fin_audiencia').val().split(':')[1]);
    if (hourFin < hour || (hourFin === hour && minuteFin < min)) {
        // Subir hora fin automáticamente +1h
        hourFin = hour + 1;
        if (hourFin >= 24) hourFin = 23;
        $('#hora_fin_audiencia').val(`${hourFin.toString().padStart(2, '0')}:${min.toString().padStart(2, '0')}`);
    }
